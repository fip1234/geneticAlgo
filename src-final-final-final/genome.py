...
class Genome():
    ...
    @staticmethod
    def get_gene_spec():
        #start- adjusted some scales for better range
    ...
    def point_mutate(genome, rate, amount):
        ...
        for gene in new_genome:
            for i in range(len(gene)):
                #strt- bug!!
                #mutatuon in negative direction too rather than increasing val 
                #generates random number between -amount and +amount to add to gene val 
                if random.random() < rate:
                    gene[i] += random.uniform(-amount, amount)
                    gene[i] = np.clip(gene[i], 0.0, 0.9999) #clip=keep in range
                #end
        return new_genome
...
#start
        # Simple: cylinder if < 0.5, box if >= 0.5
        if self.link_shape < 0.5:
            ...
            cyl_tag.setAttribute("size", size)
...
#start-same for shape collision
        if self.link_shape <0.5:
            ...
            size = f"{self.link_radius*2} {self.link_radius*2} {self.link_length}"
            c_cyl_tag.setAttribute("size", size)
       
#end        
        c_geom_tag.appendChild(c_cyl_tag)
        coll_tag.appendChild(c_geom_tag)

        #start -add friction to grip mountain
        contact_tag = adom.createElement("contact")
        friction_tag = adom.createElement("friction")
        friction_tag.setAttribute("value", "50.0")
        contact_tag.appendChild(friction_tag)
        coll_tag.appendChild(contact_tag)
        #end
