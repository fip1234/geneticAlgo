...
class MountainSim(simulation.Simulation):
    ...
    def run_creature(self, cr, iterations=2400, show_gui_time=False):
        ...
        #start########## ADD HIGH FRICTION TO ALL PARTS OF MOUNTAIN
        num_links = p.getNumJoints(mountain, physicsClientId=pid)
        p.changeDynamics(mountain, -1, lateralFriction=50.0, physicsClientId=pid)  # Base
        for link_idx in range(num_links):
            p.changeDynamics(mountain, link_idx, lateralFriction=2.0, physicsClientId=pid)
        #end##########
...
if __name__ == "__main__":
    pop = population.FixedPopulation(pop_size=20) # gene_count=2)
    sim = MountainSim()
...
#from test_ga_no_thread.py
    for iteration in range(1000):
        ...
        for i in range(len(pop.creatures)):
            ...
            # now we have the parents! DELETE FOR FIXED
            ...
            # FIXED POPULATION - only motors evolve
            pass
        ...
        ###########EVOLVE ONLY MOTOR CONTROLS##########
        #No need for shrink and grow since morphology fixed
        # Evolve only motor controls
        pop.evolve(mutation_rate=0.2)

        # Save best creature of this generation
        max_fit = np.max(fits)
        best_idx = fits.index(max_fit)
        best_this_gen = pop.creatures[best_idx]
        
        # Save to CSV
        filename = "elite_fixed_motor_"+str(iteration)+".csv"
        genome.Genome.to_csv(best_this_gen.dna, filename)
        print("saved elite to", filename)
        
        # Track overall best creature across all generations
        if best_creature is None or max_fit > best_fitness:
            best_fitness = max_fit
            best_creature = creature.FixedCreature()
            best_creature.update_dna(best_this_gen.dna)
    ...
    plot.plot_fitness(generation_list, best_fitness_list, mean_fitness_list,
                        filename='src/fitness_fixed_motors.png')
    plot.save_table(generation_list, best_fitness_list, mean_fitness_list,
                    filename='src/results_fixed_motors.csv')
